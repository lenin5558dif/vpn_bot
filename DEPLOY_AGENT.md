...
Прежде чем что-либо делать на сервере — спроси:

1. **BOT_TOKEN** — токен Telegram-бота. Получить: написать @BotFather → `/newbot`
2. **ADMIN_IDS** — Telegram ID администратора (число). Получить: написать @userinfobot
3. **ADMIN_PASSWORD** — пароль для веб-панели (придумать, мин. 8 символов)
