...
Telegram-бот для управления VPN-доступом на базе AmneziaWG (обход DPI) с веб-панелью администратора.
...
- Администрирование через бота и веб-панель: пиры, трафик, бан, скорость
...
- Настраивает fail2ban с прогрессивным баном SSH:
  - 5 неудачных попыток за 10 мин → **30 минут**
  - 2 бана за 24 ч → **24 часа**
  - 4 бана за 7 дней → **навсегда** (без восстановления)
...
- Frontend: все данные экранируются через `esc()` (защита от XSS)
